...
public abstract class Converter {
    ...
    public static FlagListDataObject toDataObject(RegionFlag[] flags) {
        ...
        teleport.put(X_TAG, tpFlag.position.getX());
        teleport.put(Y_TAG, tpFlag.position.getY());
        teleport.put(Z_TAG, tpFlag.position.getZ());
    ...
    public static RegionFlag[] fromDataObject(FlagListDataObject dataObject) {
        ...
        for (int i = 0; i < state.length; ++i) {
            if (i == FLAG_TELEPORT) {
                float x = ((Number) teleportData.get(X_TAG)).floatValue();
                float y = ((Number) teleportData.get(Y_TAG)).floatValue();
                float z = ((Number) teleportData.get(Z_TAG)).floatValue();
                String level = (String) teleportData.get(LEVEL_TAG);
                flags[i] = new RegionTeleportFlag(state[i], new Vector(x, y, z), level);
                continue;
            }
            if (i == FLAG_SELL) {
